...
export namespace Manifest {
  ...
  export const conclude = (meta: PackageJson, prop = 'cordis'): Manifest => {
    const base = meta[prop] as Manifest | undefined
    return {
      ...concludeExport(base, meta.description),
      hidden: Ensure.boolean(base?.hidden),
      preview: Ensure.boolean(base?.preview),
      insecure: Ensure.boolean(base?.insecure),
      category: Ensure.string(base?.category),
      public: Ensure.array(base?.public),
      ecosystem: Ensure.object(base?.ecosystem, (ecosystem) => ({
      ...
      exports: Ensure.dict(base?.exports, concludeExport),
    }
  }
}
