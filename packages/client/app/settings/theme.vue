  ...
  <schema-base>
    ...
    <template #control>
      <el-select popper-class="theme-select" v-model="model">
        <template v-for="(_, key) in ctx.client.theme._themes" :key="key">
          <el-option :value="key" v-if="key.endsWith('-' + schema.meta.extra.mode)">
            <div class="theme-root" :class="key.endsWith('-dark') ? 'dark' : 'light'" :theme="key">
              ...
              <div class="theme-title">
                {{ tt(ctx.client.theme._themes[key].name) }}
              </div>
            </div>
          </el-option>
        </template>
...
import { useContext, useI18nText } from '@cordisjs/client'
...
const model = computed({
  get() {
    return tt(ctx.client.theme._themes[config.value]?.name)
  },
  set(value) {
